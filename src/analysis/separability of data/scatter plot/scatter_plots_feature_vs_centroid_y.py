...
Description:
This script reads a CSV file, selects specific columns, and creates scatter plots of these features against the 'centroid_y' column. The plots are displayed and can be saved as image files.
...
>>> scatter_plots_feature_vs_centroid_y(df)
...
def scatter_plots_feature_vs_centroid_y(df):
    """
    Creates scatter plots of selected features vs 'centroid_y' and saves the plots as an image file.
    ...
    None
    """
    # Convert 'centroid_y' column to numeric type
    df['centroid_y'] = pd.to_numeric(df['centroid_y'])
    ...
    for i, feature in enumerate(selected_columns):
        plt.subplot(2, 2, i + 1)
        plt.scatter(df[feature], df['centroid_y'], alpha=0.5)
        plt.title(f'Scatter Plot of {feature} vs centroid_y')
        plt.xlabel(feature)
        plt.ylabel('centroid_y')
        plt.ylim(df['centroid_y'].min() - 0.1, df['centroid_y'].max() + 0.1)

    # Adjust layout and save the figure as a PNG file
    plt.tight_layout()
    plt.savefig('results/figure/scatter plots/scatter_plots_feature_vs_centroid_y.png', format='png', dpi=300)
    plt.show()
